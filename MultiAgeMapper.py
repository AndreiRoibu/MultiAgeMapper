...
class AgeMapper_input1(nn.Module):
    def __init__(self, 
        ...
        if network_parameters is not None:
            ...
            dropout_fc1_flag = network_parameters['dropout_fc1_flag']
            dropout_fc2_flag = network_parameters['dropout_fc2_flag']
            dropout_fc3_flag = network_parameters['dropout_fc3_flag']
        else:
            norm_flag = None
            nonlin_flag = None
            dropout_fc1_flag = 0
        ...
        if dropout_fc2_flag > 0:
            self.FullyConnected.add_module(
                name='Dropout_FullyConnected_2',
                module=nn.Dropout(dropout_fc2_flag)
            )
        ...
        if dropout_fc3_flag > 0:
            self.FullyConnected.add_module(
                name='Dropout_FullyConnected_1',
                module=nn.Dropout(dropout_fc3_flag)
            )
...
class AgeMapperConvFC2(nn.Module):
    def __init__(
        ...
        if dropout_fc2_flag > 0:
            self.FullyConnectedModality.add_module(
                name='Path_%d_Dropout_FullyConnected_2' % (path_number),
                module=nn.Dropout(dropout_fc2_flag)
            )
    ...
    def __init__(self, 
        ...
        for idx in range(self.input_paths):
            self.Modality_Paths[str(idx)] = AgeMapperConvFC2(channel_number=channel_number,
                                                            ...
                                                            norm_flag=norm_flag,
                                                            nonlin_flag=nonlin_flag,
                                                            dropout_fc1_flag=dropout_fc1_flag,
                                                            dropout_fc2_flag=dropout_fc2_flag
                                                            )
